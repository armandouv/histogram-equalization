...
double equalize_image_sequential(unsigned char *image, int width, int height, int number_of_channels, char *image_name)
{
    ...
    get_cumulative_distribution_sequential(histogram);
    long cumulative_distribution_minimum = get_first_nonzero(histogram);
    get_equalized_cumulative_distribution_sequential(histogram, cumulative_distribution_minimum, size);
    generate_image_sequential(image, new_image, number_of_channels, histogram, size);
