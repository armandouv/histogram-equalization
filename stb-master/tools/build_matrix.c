...
enum
{
    NONE, IN_COMPILERS, IN_ARGS, IN_PROJECTS, IN_LINK
};
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        else if (0 == stricmp(p, "[compilers]"))
        { state = IN_COMPILERS; }
        else if (0 == stricmp(p, "[args]"))
        {
            state = IN_ARGS;
            shared_args = NULL;
        }
        else if (0 == stricmp(p, "[projects]"))
        { state = IN_PROJECTS; }
        else if (0 == stricmp(p, "[link]"))
        {
            state = IN_LINK;
            shared_link = NULL;
        }
        else
        {
            ...
            {
                case NONE:
                    stb_fatal("Invalid text outside section at line %d.", i + 1);
                case IN_COMPILERS:
                {
                    char buffer[4096];
                    int count;
